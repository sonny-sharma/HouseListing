...
const EditListing = () => {
  ...
  // redirect if list doesn't belong to user

  useEffect(() => {
    if (listing && listing.userRef !== auth.currentUser.uid) {
      toast.error(`You can't edit this List`);
      navigate("/");
    }
  ...
  const onSubmit = async (e) => {
    ...
    if (discountedPrice >= regularPrice) {
      setLoading(false);
      toast.error("discountedPrice should be less than regular price");
      return;
    }

    if (images.length > 6) {
      setLoading(false);
      toast.error("Only 6 images can be Entered");
      return;
    }
